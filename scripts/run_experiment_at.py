...
class NoiseConfig:
    ...
    def _validate(self):
        ...
        print(f"Noise type: {self.noise_type}, Noise level: {self.noise_level}")
...
def run_experiment(experiment_config: Dict[str, Any], output_dir: str, experiment_name: str, noise_type=None, noise_level=None) -> Dict[str, Any]:
    ...
    noise_config = NoiseConfig(noise_type=noise_type, noise_level=noise_level)
    
    # Run evaluation for each model
    for model in models:
        logger.info(f"Evaluating model: {model}")
        model.add_noise(noise_config)
...
def main():
    ...
    parser.add_argument("--noise-type", default="gaussian", help="Type of noise to add (e.g., gaussian, uniform)")
    parser.add_argument("--noise-level", type=float, default=1e-3, help="Level of noise to add")
